...
def main(args=None):
    ...
    for run_path in run_paths:
        ...
        output_path = run_path.with_stem(f"__novelty__{run_path.stem}").with_suffix(
            ".jsonl.gz"
        )
        run.to_json(
            output_path,
    ...
    for qrel_id in qrel_ids:
        ...
        output_path = (args.output_dir / qrel_id.replace("/", "-")).with_suffix(
            ".qrels"
        )
        qrels[["query_id", "iteration", "doc_id", "relevance"]].dropna(
            subset="relevance"
